...
export const createOffer = asyncHandler(async (req, res) => {
  ...
  // Check for duplicate offer code (optional early check)
...
export const updateOffer = asyncHandler(async (req, res) => {
  ...
  const { code, offerType, offerValue, startDate, endDate, description } =
    req.body;
...
export const listUnListOffer = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: `Offer${offer.isBlocked ? "Blocked" : "Unblocked"} successfully`,
      offer,
    });
  } catch (error) {
    res
